...
module.exports = function (app, requireUser) {
  ...
  router.post("/create", requireUser, async (req, res) => {
    try {
      ...
      // Validate Stock and Quantities Before Order Creation
      for (const item of normalizedProducts) {
        if (item.qty < 1) {
          return res.status(400).json({ message: "Minimum quantity per order is 1" });
        }
        if (item.qty > 3) {
          return res.status(400).json({ message: "Maximum quantity per product is 3" });
        }
        let book;
      ...
      for (const item of normalizedProducts) {
        ...
        if (book) {
          ...
          if (book.stockQuantity <= 5) {
            ...
            // Admin Email Alert for Low Stock
          ...
          // Update Seller metrics if sellerId exists
          if (book.sellerId) {
